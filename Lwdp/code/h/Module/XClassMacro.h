...
public: \
    const tchar_* DoGetClassName() const { return __T(#cls); }    \
    static bool DoQueryInterface(cls* self, X3IID iid, Ix_Object** ppv, const MODID& fromModule)  \
    {
...
#define X3USE_INTERFACE_ENTRY(_BaseClass)       \
        if (_BaseClass::DoQueryInterface(self, iid, ppv, fromModule))  \
        {   \
            return true;    \
        }
